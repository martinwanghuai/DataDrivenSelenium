...
public class MathUtils {
	...
	public static synchronized String printDescriptiveStatistic(ConcurrentLinkedDeque<Double> objs) {
		...
		sb.append(stats.getN()).append("\t").append(df.format(stats.getMean())).append("\t")
				.append(df.format(stats.getPercentile(50))).append("\t").append(df.format(stats.getPercentile(90)))
				.append("\t").append(df.format(stats.getPercentile(95))).append("\t")
				.append(df.format(stats.getPercentile(99))).append("\t").append(df.format(stats.getMin())).append("\t")
				.append(df.format(stats.getMax())).append("\t\n");
	...
	public static boolean isWithinRange(double a, double b, double range) {
		...
		return (BigDecimal.valueOf(a).subtract(BigDecimal.valueOf(b))).abs().subtract(BigDecimal.valueOf(range))
				.signum() <= 0;
	}
	...
	public static double irr(double[] values) {
		...
		while (i < maxIterationCount) {
			...
			// the value of the function (NPV) and its derivate can be calculated in the
			// same loop
			double fValue = 0;
	...
	public static double calculateRate(double loanAmount, double loanTenure, double monthlyRepayment) {
		/*
		 * Instead of iterating until a guess is found, we have decided to only iterate
		 * a max of 20 or double the tenure which should be enough to find the correct
		 * value
		 */
